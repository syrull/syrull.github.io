...
```bash
❯ cat Packs/data.wdt | xxd | head -n 50 
...
```bash
❯ strings Packs/data.wdt | head -n 50  
LZSSV
8C /
B~pUjo
)}XE
...
```
❯ go run cmd/main.go -d ~/dev/tzar-re/Tzar/Packs/data.wdt 
...
```bash
❯ strings ./Packs/data.wdt.decompressed | head -n 50
...
```c
if (-1 < iVar2)  // if MSB is 0 → back-reference
    ...
else             // if MSB is 1 → literal byte
...
Bit stream: [1][8-bit literal] or [0][12-bit offset][4-bit length]
            │                      │  └─ offset into sliding window (0-4095)
            │                      └─ length of match (encoded, +2 minimum)
            └─ type bit: 1=literal, 0=reference
```
...
```bash
❯ uv run ./tzar_wdt_decompress.py /home/syl/dev/tzar-re/Tzar/Packs/data.wdt data_raw.bin
...
<CodeScript title="LZSS Decompressor" filename="tzar_wdt_lzss_decompress.py" lang="python">
{`
#!/usr/bin/env python3
"""
...
Now we have another challenge... What the hell is `HMMSYS PackFile` 😭.
...
<CodeScript title="WDT Extractor" filename="tzar_wdt_extractor.py" lang="python">
{`
#!/usr/bin/env python3
"""
...
<CodeScript title="Tzar WDT Tool" filename="tzar_wdt_tool.py" lang="python">
{`
#!/usr/bin/env python3
"""
